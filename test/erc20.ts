...
describe('Token', function () {
  ...
  describe('Transaction', () => {
    ...
    it("Should fail if send doesn't have enugh tokens", async () => {
      const initialOwnerBalance = await ctx.token.balanceOf(
        ctx.deployer.address
      )
      await expect(
        ctx.token.connect(ctx.user1).transfer(ctx.deployer.address, 1)
      ).to.be.revertedWith('ERC20: transfer amount exceeds balance')

      expect(await ctx.token.balanceOf(ctx.deployer.address)).to.equal(
        initialOwnerBalance
      )
    })
  })
  ...
  describe('Burned', () => {
    it('should burned tokens', async () => {
      ...
      const userBalanceAfterBurned = await ctx.token.balanceOf(
        ctx.user1.address
      )
    ...
    it("Should fail burned doesn't have enough tokens", async () => {
      ...
      await expect(
        ctx.token.connect(ctx.user1)._burn(amount)
      ).to.be.revertedWith('ERC20 not enough funds')
    })
    it('Should burned token from user 1', async () => {
      ...
      expect(
        await ctx.token.allowance(ctx.deployer.address, ctx.user1.address)
      ).to.equal(0)
    })
    it('Should fail not enough funds on account', async () => {
      ...
      expect(
        await ctx.token
          .connect(ctx.deployer.address)
          .allowance(ctx.user1.address, ctx.deployer.address)
      ).to.equal(50)
      await expect(
        ctx.token.burnFrom(ctx.user1.address, amount)
      ).to.be.revertedWith('ERC20: not enough funds on account')
    })
  })
  ...
  describe('Approve', () => {
    ...
    it("Should doesn't approve zero address", async () => {
      await expect(
        ctx.token.approve(ethers.constants.AddressZero, 5)
